...
def get_candlestick():
    ...
    rates = mt5.copy_rates_range(symbol, timeframe_h4, start_time, end_time)
    ...
    ohlc_data_h4 = df[['time', 'open', 'high', 'low', 'close']]

    ohlc_data_h4.to_csv('ohlc_data_h4.csv', index=False, header=False)
        ...
        for position in positions:
            if position.type == mt5.ORDER_TYPE_BUY:
                if expert_result == "buy":
                    ...
                    close_trade(ticket, 'sell')
                    open_trade('sell')
                elif expert_result == "hold":
                    print('Expert result = HOLD')
            elif position.type == mt5.ORDER_TYPE_SELL:
                if expert_result == "sell":
                    print("No Modification")
                elif expert_result == "buy":
                    close_trade(ticket, 'buy')
                    open_trade('buy')
                elif expert_result == "hold":
                    print("You should hold your position!")
